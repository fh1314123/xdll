...
exampaper_add = {
						...
						success : function(message, tst, jqXHR) {
							...
							if (message.result == "success") {
								util.success("添加成功", function() {
									document.location.href = document.getElementsByTagName('base')[0].href + 'admin/toExamPaperPreview/'
									+message.paperId;
								});
							} else {
		...
		composeEntity : function composeEntity(){
			...
			paperParam.duration = $(".add-update-duration input").val();
			paperParam.sum_score = $("#total-point").val();
			paperParam.paperType = $(".add-update-exampaper-type select").val();
			paperParam.difficulty = $(".add-update-difficulty input").val();
			var qt = $(".add-ques-type");
